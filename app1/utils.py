...
def integrateFunction(func):
    ...
    if (len(changedlist[1]) == 3):
        ...
        result = float(sp.integrate(data.get("function"), (data.get(
            "variable"), data.get("starting"), data.get("ending"))))
        return result
    else:
        data["function"] = changedlist[0]
        data["variable"] = str(changedlist[1][0])
        print(data)
        result = sp.integrate(data.get("function"),
                              sp.Symbol(data.get("variable")))
        return result
...
class MidPoint:
    ...
    def midpointmaxInainterval(self, secondDerivative):
        ...
        # max value
        res = minimize_scalar(
            lambda su: -self.f(su, secondDerivative), bounds=interval, method='bounded')
        # min value
        res1 = minimize_scalar(lambda su: self.f(
            su, secondDerivative), bounds=interval, method='bounded')
...
# ============================================================================================================================
    ...
    def midpointError(self):
        # k*(b-a)**3/(24*(n)**2)
        erroranswer = float(self.k*(self.ending-self.starting)
                            ** 3)/(24*(self.interval)**2)
        return erroranswer

    def numberofInterval(self):
        numberofIntervalanswer = math.sqrt(
            (self.k*(self.ending-self.starting)**3)/(24*self.error))
...
# ============================================================================================================================
    ...
    def trapezoidError(self):
        # k*(b-a)**3/(24*(n)**2)
        erroranswer = (self.k*(self.ending-self.starting)**3) / \
            (12*(self.interval)**2)
        return erroranswer

    def numberofInterval(self):
        numberofIntervalanswer = math.sqrt(
            (self.k*(self.ending-self.starting)**3)/(12*self.error))
        return math.ceil(numberofIntervalanswer)
...
# ============================================================================================================================
    ...
    def simpsonError(self):
        # k*(b-a)**3/(24*(n)**2)
        erroranswer = (self.k*(self.ending-self.starting)**5) / \
            (180*(self.interval)**4)
        return erroranswer

    def numberofInterval(self):
        numberofIntervalanswer = (
            (self.k*(self.ending-self.starting)**5)/(180*self.error))**(1/4)
